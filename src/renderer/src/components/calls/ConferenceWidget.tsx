  ...
  async function initConference(ctrl: AbortController) {
    try {
      ...
      try {
        ...
        // No microphone available — join with an empty stream so user can still listen
        stream = new MediaStream()
        setMuted(true)
      }
      // Guard against React Strict Mode double-mount: if cleanup already ran, bail out
