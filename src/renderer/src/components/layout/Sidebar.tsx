...
import {
  Home, Headphones, CheckSquare, Bell, Settings,
...
export function Sidebar({
  tab, setTab, auth, onLogout, isOnline, messageBadge, messageMention, updateBadge,
}: {
...
function StatusIcon({ size = 10, border, isOnline }: { size?: number; border: string; isOnline: boolean }) {
  return (
    <span style={{
    ...
    <div ref={containerRef} style={{ position: 'relative' }}>
      ...
      >
        ...
        <span style={{ position: 'absolute', bottom: -2, right: -2 }}>
          <StatusIcon size={10} border={C.bgTertiary} isOnline={isOnline} />
        </span>
      </button>
      ...
      {hovered && !menuOpen && (
        <div style={{
          ...
          <StatusIcon size={8} border={C.bgFloating} isOnline={isOnline} />
        </div>
      )}
